...
from Problem5Inputs import HealthStates
...
class Patient:
    ...
    def simulate(self, n_time_steps):
        ...
        while self.stateMonitor.get_if_alive() and k < n_time_steps:
            if self.stateMonitor == HealthStates.WELL or HealthStates.POST_STROKE:

                # sample from the Markov jump process to get a new state
            # (returns an integer from {0, 1, 2, ...})
                new_state_index = markov_jump.get_next_state(current_state_index=self.stateMonitor.currentState.value,
                                                             rng=rng)

            self.stateMonitor.update(time_step=k, new_state=HealthStates(new_state_index))


            if self.stateMonitor.currentState == HealthStates.STROKE:
...
class PatientStateMonitor:
    ...
    def update(self, time_step, new_state):
        ...
        if new_state == self.temporaryState:
            self.strokeNumber += 1

        # if self.currentState == HealthStates.POST_STROKE and new_state == HealthStates.STROKE:
        #     self.strokeNumber +=1
...
class CohortOutcomes:
    ...
    def calculate_cohort_outcomes(self, initial_pop_size):
        ...
        # calculate mean time to Post Stroke
        self.meanTimeToPostStroke = sum(self.timesToPostStroke) / len(self.timesToPostStroke)

        self.totalStrokes = sum(self.totalNumberOfStrokes)
